...
//	}
	...
	if (_mytree_node -> next != 0) {
		if (_mytree_node -> next -> ptr != 0) {
			free(_mytree_node -> next -> ptr);
		}
	...
	if (_mytree_path != 0) {
		...
		for (; _i > 0; _i = _i - 1) {
//			printf("%d", _i);	printf("\n");
//			printf("%d", _mytree_path -> node -> next -> ptr[_i-1]);	printf("\n");
			if (_mytree_path -> node -> next -> ptr[_i-1] != 0) {
				break;
			}
		}
...
//				printf("%d", _tmplength);	printf("\n");
				...
				_i = 0;
				for ( ; _i <= _tmplength; ) {
					_newarray[_i] = _mytree_node -> next -> ptr[_i];
					_i++;
				}
	
				free(_mytree_node -> next -> ptr);
				_mytree_node -> next -> ptr = _newarray;
			}
		}
	}
