...
/*
	...
	if (mytree > 0)		// skip null-valued trees
	{
		for(;;)
		{
			...
			if (_tmplength > 0)
			{
				...
				if (_mytree_path == 0) 				// found the tree root
				{
					mytree = 0;
					_mytree_node = mytree;
					break;
					...
					_i = _mytree_path -> node -> length;
					_i = _i - 1;

					for(;_i >= 0; _i = _i - 1)
					{
						_comparenode = _mytree_path -> node -> ptr[_i];
	
						if (_comparenode == _mytree_node)
						{
							_mytree_path -> node -> ptr[_i] = 0;
							break; 
						}
